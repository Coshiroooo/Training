...
public class Main {
	...
	static void printQuestion(int n, int logId, int questionId) {
		...
		List<Object> choiceList = choicesFilter(questionId).get("choice");
		
		quizDB.update(String.format("UPDATE logs SET questions_id = %s WHERE id = %s", questionId, logId));
		
		System.out.println("【第" + n + "問】　" + questions.get("question").stream().map(q -> q.toString()).collect(Collectors.toList()).get(questionId - 1));
