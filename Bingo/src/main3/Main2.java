...
public class Main2 {
	...
	public static void makeBingo() { // ビンゴを作る
		...
		String judge = (winningNumbers.get(winningNumbersIndex) == -1) ? ""
				: (winningNumber == winningNumbers.get(winningNumbersIndex)) ? "当たり！" : "残念！";
		System.out.println("\r\n" + "\r\n" + judge + "\r\n");
	...
	public static boolean bingoJudge() { // ビンゴを判定する機能
		...
		for (List<Integer> matrix1 : matrixList) { // ビンゴ列になりうるリストを作り、その中に既出の当選番号がカードの幅数と同じだけあるかどうかを繰り返しで検証する
			...
			if (countSide == squareLength || countVer == squareLength || countSlant1 == squareLength
					|| countSlant2 == squareLength) { // マス目の幅と同じ数あったらビンゴ
				isBingoJudge = true;
				break;
			}
		}
